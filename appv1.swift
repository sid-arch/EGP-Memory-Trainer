...
enum ConstantKind: String, Codable, CaseIterable, Identifiable {
    ...
    // ASCII filenames so we don't write π/φ to disk names
    var fileName: String {
        switch self {
...
final class SessionStore: ObservableObject {
    ...
    init() {
        ConstantKind.allCases.forEach { kind in
...
final class DigitSpeechRecognizer: ObservableObject {
    ...
    private let recognizer: SFSpeechRecognizer? = {
        ...
        // Optional: gives the recognizer a “dictation” hint on newer iOS
    ...
    var body: some View {
        VStack(spacing: 18) {
            HStack(alignment: .top, spacing: 24) {
                ...
                // LEFT: Title, counter, transcript, stats
                VStack(spacing: 24) {
                    ...
                    // Monkeytype-style stream
                    ScrollView(.horizontal) {
                        HStack(spacing: 4) {
                            ForEach(transcript) { token in
                                switch token.kind {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    Text("Session Log").font(.title2).bold()
                    if let list = store.records[constant], !list.isEmpty {
                        ScrollView {
                            VStack(alignment: .leading, spacing: 10) {
                                ForEach(list) { rec in
                                    SessionRow(record: rec)
                                        .onTapGesture { lastRecord = rec; showResults = true }
                                }
                            }
                        }
                    } else {
