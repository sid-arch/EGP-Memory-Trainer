...
// Grey and Underline for digits that are recitded, but not transcribed
...
struct TrainerView: View {
    ...
    // MARK: - Processing (Strict Sequential, Teacher-Style)
    ...
    private func processIncomingDigit(_ d: String) {
        ...
        if d == expected {
            ...
            transcript.append(GradedToken(kind: .digit(d: d, correct: false)))
            wrong += 1
            if wrong >= 10 { endSession(auto: true); return }
        }

        digitCount += 1 // Always move forward
    }
